...
const experiences = [
  {
    ...
    highlights: [
      ...
      "Automated 65% of customer service operations measured by response time reduction from hours to minutes across 3,000+ annual service calls, by leveraging Claude Code for rapid Convex development and architecting n8n workflows for scheduling, payment reminders, and ElevenLabs voice agent integration"
    ],
    technologies: ["React", "TypeScript", "Convex", "Google Calendar API", "SMS/Email APIs", "Real-time Backend"],
    gradient: "from-violet-600 to-indigo-500 dark:from-violet-400 dark:to-indigo-300",
          ...
          <div className="space-y-12">
            {experiences.map((exp, index) => (
              <div key={exp.role} className="relative group">
                {/* Timeline dot */}
                <div className={`absolute left-6 w-4 h-4 rounded-full bg-gradient-to-r ${exp.gradient} shadow-lg group-hover:bg-gradient-to-r group-hover:from-primary group-hover:to-teal-400 dark:group-hover:from-teal-400 dark:group-hover:to-purple-400 transition-all duration-300`} />
                
                <Card className="ml-16 hover:shadow-2xl transition-all duration-500 hover:scale-[1.02] bg-background/80 backdrop-blur-sm border-border hover:border-primary/30">
                  <CardHeader>
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div>
                        <CardTitle className={`text-2xl transition-all duration-300 ${exp.hoverColor}`}>
                          {exp.role}
                        </CardTitle>
